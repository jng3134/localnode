...
export default function App() {
  ...
  const isProjectDashboardView = Boolean(activeProjectId && !activeConversationId);
  ...
  return (
    <div id="studio-app-root" className="flex h-screen w-screen overflow-hidden bg-[linear-gradient(135deg,#120536_0%,#2F1387_40%,#5b3bc9_70%,#c7d2fe_100%)] text-[#EDEDED] font-sans selection:bg-indigo-500/30 relative">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-transparent z-10 w-full">
        ...
        <header className="flex-shrink-0 h-[72px] border-b border-white/[0.07] bg-white/[0.01]/75 backdrop-blur-md flex items-center justify-between px-6 md:px-8 relative z-10 transition-colors">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col border-l border-white/10 pl-5 hidden sm:flex">
              <span className="text-[10px] text-zinc-400 uppercase font-bold tracking-widest font-sans">
                {isProjectDashboardView ? 'Workspace Context' : 'Active Thread'}
              </span>
              
              {isProjectDashboardView ? (
                 <div className="mt-0.5 font-medium text-sm text-[#EDEDED]">
                   Project Overview: <span className="text-zinc-300">{activeProject?.name}</span>
                 </div>
              ) : headerEditing ? (
                <input
                  type="text"
        ...
        {isProjectDashboardView ? (
          ...
          <div className="flex-1 overflow-hidden flex flex-col relative bg-transparent w-full">
